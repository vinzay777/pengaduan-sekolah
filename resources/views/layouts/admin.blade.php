    ...
    <div class="flex gap-3 sm:gap-4 max-w-[1600px] mx-auto">
        ...
        <div id="sidebar" class="fixed top-0 left-0 h-screen z-50 w-64 bg-gradient-to-b from-orange-400 via-orange-500 to-orange-600 p-6 flex flex-col shadow-xl -translate-x-full transition-transform duration-300 ease-in-out
            ...
            <div class="flex-1">
                ...
                <ul class="space-y-2">
                    <li>
                        <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-3 text-white {{ request()->routeIs('admin.dashboard')}} px-4 py-3 rounded-lg font-medium transition">
                            <i data-lucide="layout-dashboard" class="w-5 h-5"></i>
                            <span>Dashboard</span>
                        </a>
                    </li>
                    <li>
                        <a href="{{ route('admin.kelola-pengaduan') }}" class="flex items-center gap-3 text-white {{ request()->routeIs('admin.kelola-pengaduan')}} px-4 py-3 rounded-lg font-medium transition">
                            <i data-lucide="clipboard-list" class="w-5 h-5"></i>
                            <span>Kelola Pengaduan</span>
                        </a>
                    </li>
                    <li>
                        <a href="{{ route('admin.kategori.index') }}" class="flex items-center gap-3 text-white {{ request()->routeIs('admin.kategori.*') ? 'bg-white/20' : '' }} px-4 py-3 rounded-lg font-medium hover:bg-white/20 transition">
                            <i data-lucide="tags" class="w-5 h-5"></i>
                            <span>Kelola Kategori</span>
                        </a>
                    </li>
                    <li>
                        <a href="{{ route('admin.siswa.index') }}" class="flex items-center gap-3 text-white {{ request()->routeIs('admin.siswa.*') ? 'bg-white/20' : '' }} px-4 py-3 rounded-lg font-medium hover:bg-white/20 transition">
                            <i data-lucide="users" class="w-5 h-5"></i>
                            <span>Kelola Siswa</span>
                        </a>
                    </li>
                    {{-- <li>
                        <a href="{{ route('admin.pengaduan') }}" class="flex items-center gap-3 text-white {{ request()->routeIs('admin.pengaduan') ? 'bg-white/20' : '' }} px-4 py-3 rounded-lg font-medium hover:bg-white/20 transition">
                            <i data-lucide="plus-circle" class="w-5 h-5"></i>
                            <span>Buat Pengaduan</span>
                        </a>
                    </li> --}}
                    {{-- <li>
                        <a href="{{ route('admin.histori') }}" class="flex items-center gap-3 text-white {{ request()->routeIs('admin.histori') ? 'bg-white/20' : '' }} px-4 py-3 rounded-lg font-medium hover:bg-white/20 transition">
            ...
            <!-- User Info & Logout -->
            <div class="mt-auto">
                <a href="{{ route('admin.profile') }}" class="block bg-white/10 rounded-lg p-4 mb-3 hover:bg-white/20 transition">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center">
                            <i data-lucide="user" class="w-5 h-5 text-white"></i>
                        </div>
                        <div class="flex-1 min-w-0">
                            <p class="text-white font-semibold text-sm truncate">{{ Auth::guard('admin')->user()->nama }}</p>
                            <p class="text-orange-100 text-xs font-medium">Admin / {{ Auth::guard('admin')->user()->jabatan }}</p>
                        </div>
                    </div>
                </a>
